...
def _build_rationale(goal_state: TrainingState, nodes_explored: int) -> str:
    """Build a human-readable description of the plan found.

    Args:
        goal_state: The goal state containing the complete plan.
        nodes_explored: Number of states expanded during search.
    ...
    parts = [
        f"{total_weeks}-week plan",
        f"peaking at {peak_miles:.1f} miles/week",
        f"using {', '.join(sorted(terrain_used))} terrain",
        f"for a {goal_state.experience} runner",
    ]
    return (
        " ".join(parts) + ". "
