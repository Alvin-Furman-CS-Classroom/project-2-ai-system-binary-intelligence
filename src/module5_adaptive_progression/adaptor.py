...
def _pretrain_on_history(engine: QLearningEngine, ctx: dict) -> None:
    """
    Bootstrap the Q-table from the run history already present in ctx.

    For each run[i] in history we treat history[:i] as the pre-run context
    and run[i] as the outcome, then apply one Q-learning update.  This gives
    the engine a non-zero starting point so Q-values are meaningful on the
    very first inference call.
    """
    history = ctx["history"]
    if not history:
        return

    adherence = _get_adherence(ctx)

    for i, run in enumerate(history):
        sub_history = history[:i]
        sub_ctx = {**ctx, "history": sub_history}

        weekly = estimate_weekly_miles(sub_history)
        state  = _build_state(sub_ctx, weekly)

        base_dist, _ = extract_baseline(sub_history)

        # Map history-run fields to the outcome format _infer_action_from_outcome expects
        outcome = {
            "distance_completed": run.get("distance", base_dist),
            "sentiment":          run.get("sentiment", "neutral"),
        }
        action = _infer_action_from_outcome(outcome, base_dist)

        terrain_before = history[i - 1]["terrain"] if i > 0 else ctx["terrain"]
        terrain_after  = run.get("terrain", ctx["terrain"])
        sentiment_canonical = normalize_sentiment_to_canonical(run.get("sentiment", "neutral"))

        reward = compute_reward(
            sentiment            = sentiment_canonical,
            fatigue_score_before = ctx["fatigue_score"],
            fatigue_score_after  = ctx["fatigue_score"],
        ...
        next_terrain_seq = get_terrain_sequence(sub_history) + [terrain_after]
        next_weekly      = weekly + float(run.get("distance", base_dist))
    ...
    # Anchor step: train directly on the inference state (full history) so that
    # confidence() finds Q-entries for the exact state key used at inference time.
    # Re-use the last run as the outcome; next_state is a self-loop for the
    # terminal state (no future run available).
    last_run   = history[-1]
    inf_weekly = estimate_weekly_miles(history)
    inf_state  = _build_state(ctx, inf_weekly)
    base_dist, _ = extract_baseline(history)

    anchor_outcome = {
        "distance_completed": last_run.get("distance", base_dist),
        "sentiment":          last_run.get("sentiment", "neutral"),
    }
    action = _infer_action_from_outcome(anchor_outcome, base_dist)

    terrain_before      = history[-2]["terrain"] if len(history) >= 2 else ctx["terrain"]
    terrain_after       = last_run.get("terrain", ctx["terrain"])
    sentiment_canonical = normalize_sentiment_to_canonical(last_run.get("sentiment", "neutral"))
    ...
    engine.update(inf_state, action, reward, inf_state)
    engine.end_episode()
...
def _snapshot_q_values(engine, state) -> dict:
    return {
        f"{a[0].name.title()} / {a[1].name.title()}": round(engine._q_value(state, a), 4)
...
def adapt_progression(context):
    ...
    _pretrain_on_history(engine, ctx)
